...
/* Non-zero if we have just funwindorked and are currently running in a subshell
   environment. */
...
/* Execute the command passed in COMMAND.  COMMAND is exactly what
   ...
   return values.  Executing a command execute_commandwith nothing in it returns
   EXECUTION_SUCCESS. */
int
	...
	if (already_making_children && pipe_out == NO_PIPE)
	  {
	    ...
	    /* When executing a shell function that executes other
	       ...
	       pipelinedo_redirections (command->redirects, RX_ACTIVE|RX_UNDOABLE)s) to be waited for twice. */
